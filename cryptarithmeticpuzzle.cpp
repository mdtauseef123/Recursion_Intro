#include<bits/stdc++.h>
using namespace std;
bool isAnyMapping(vector <string> &words,int row,int col,int bal,unordered_map<char,int> &letToDig,vector<char> &digToLet,int totalRows,int totalCols) {
    ...
    if(row == totalRows){
        return ((bal % 10 == 0) and isAnyMapping(words, 0, col + 1, bal / 10, letToDig, digToLet, totalRows, totalCols));
    }
    string w = words[row];
